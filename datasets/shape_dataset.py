...
class SingleShapeDataset(Dataset):
    def __init__(self,
                 ...
                 return_corr=True, return_dist=False, randm_rotation = False):
        """
        ...
        self.randm_rotation = randm_rotation
    ...
    def __getitem__(self, index):
        ...
        # 设置随机旋转, 随着y轴
        if self.randm_rotation :
            verts = self._centering(verts)
            item['verts'] = self._random_rotation(verts, self.randg)

        item['verts'] = torch.from_numpy(verts).float()  # 声明成torch
    ...
    # 给smal 添加的旋转
    def _centering(self, vertices):
            center = np.mean(vertices, axis=0, keepdims=True)
            return vertices - center

    def _random_rotation(self, vertices, randg, axes ='y'):
        # vertices = sdict['vertices']
        dtype = vertices.dtype
        # max_x, max_y, max_z = args
        max_x, max_y, max_z = 360.0, 360.0, 360.0
...
class SingleFaustDataset(SingleShapeDataset):
    def __init__(self, data_root,
        ...
        super(SingleFaustDataset, self).__init__(data_root, return_faces,
                                                 ...
                                                 return_corr, return_dist, randm_rotation)
...
class SingleShrec19Dataset(SingleShapeDataset):
    def __init__(self, data_root,
                 return_faces=True,
                 return_evecs=True, num_evecs=200,
                 return_dist=False, randm_rotation = False):
        super(SingleShrec19Dataset, self).__init__(data_root, return_faces, return_evecs, num_evecs, False, return_dist, randm_rotation)



# add rotation 
@DATASET_REGISTRY.register()
class SingleSmalDataset(SingleShapeDataset):  # 继承类的方法？
    def __init__(self, data_root, phase='train', category=True,
                 return_faces=True,
                 return_evecs=True, num_evecs=200,
                 return_corr=True, return_dist=False, randm_rotation = True):
        self.phase = phase
...
class PairShrec19Dataset(Dataset):
    def __init__(self, data_root, phase='test',
        ...
        self.dataset = SingleShrec19Dataset(data_root, return_faces, return_evecs, num_evecs, return_dist, randm_rotation)
        self.phase = phase
        if phase == 'test':
...
class PairSmalDataset(PairShapeDataset):
    def __init__(self, data_root, phase='train',
        ...
        dataset = SingleSmalDataset(data_root, phase, category, return_faces,
                                    return_evecs, num_evecs,
                                    return_corr, return_dist, randm_rotation)
        self.randm_rotation = True #  设置旋转for smal
